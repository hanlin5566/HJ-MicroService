 ...
 **/
@Component
public class PostZuulFilterTest extends ZuulFilter {
	...
	public Object run() {
		...
		try {
			...
			logger.error(body);
			//返回值取出后，如不再次设值，则返回内容
//			ctx.setResponseBody(ResponseData.fail(ResponseCode.RESOURCE_NOT_FOUND).toString());
		} catch (IOException e) {
			e.printStackTrace();
		}
		logger.error("后置过滤器-->网关收到应答后触发。");
		return null;
	}
